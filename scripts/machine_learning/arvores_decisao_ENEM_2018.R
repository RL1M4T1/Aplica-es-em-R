...
# pacotes
pacman::p_load(ade4, car, caret, chunked, data.table, data.tree, dplyr, ff, ffbase, foreign, funModeling, 
               ggparty, ggplot2, ggtree, gplots, LaF, Metrics, party, partykit, permimp,plm, randomForest, rattle, readr, REEMtree, sqldf) 

# usando o p_load do pacman vc garante que 1) se o pacote estiver instalado, será carregado 2) se não estiver será instalado

ENEM_ESCOLA_2018 <- read.csv2('bases_tratadas/ENEM_ESCOLA_2018.csv', stringsAsFactors = T) # carregando a base já tratada para o ambiente do R

ENEM_ESCOLA_2018

# AED 
status(ENEM_ESCOLA_2018) # explorar a qualidade das variáveis
freq(ENEM_ESCOLA_2018) # explorar os fatores
...
forestENEM = randomForest(treinoENEM[ , c(3, 8, 12)], treinoENEM[ , 4], ntree = 100, keep.forest=T, importance=T) # floresta aleatória
